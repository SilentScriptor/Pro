...
local function Obfuscate(luaCode)
    local HerculesObfuscator = SafeGet("https://raw.githubusercontent.com/zeusssz/hercules-obfuscator/main/src/obfuscate.lua")
    if HerculesObfuscator then
        local obfuscateFunc = loadstring(HerculesObfuscator)
        return obfuscateFunc and obfuscateFunc(luaCode) or luaCode
    else
        warn("❌ Failed to load Hercules Obfuscator. Running script without obfuscation.")
        return luaCode
    end
end
        ...
        Callback = function()
            ...
            if url then
                ...
                if scriptSource then
                    local obfuscatedScript = Obfuscate(scriptSource)
